...
if len(sys.argv) != 6:
  print("Usage: %s <PlanetGeneratorDefinitions.sbc> <Planet name> <Planet position: X Y Z> <Radius> <Point on cubemap (X Y) or GPS (X Y Z)>" % sys.argv[0])
  raise SystemExit
...
planet_min_hill = float(tree.xpath("//Definitions/Definition[Id/SubtypeId='%s']/HillParams" % planet)[0].attrib["Min"])
planet_max_hill = float(tree.xpath("//Definitions/Definition[Id/SubtypeId='%s']/HillParams" % planet)[0].attrib["Max"])
