 ...
 */
class GeneralSettings{public Program Context;public WcPbApi WCAPI; public bool DisplayStatusInName; public int η=600;public int ζ=45;public int ε=3600;
...
double π=36;public double ο=2000;public double ξ=0.5;public double ν=0.15;public double μ=800;public int λ=60;public int Ψ=600
...
=400;const double Ο=1000;const double Ξ=0;const double Ν=0;const bool Υ=false;const bool Μ=true;const double Κ=100;const
double Ι=600;const double Θ=200;const double Η=1000;const double Ζ=4;const double Ε=1;const bool Δ=true;const bool Α=true;class
...
string Ϗ="AUTOLAUNCH_OFF";const string ϟ="DEBUGMODE";const double Ϟ=0.707;const int ϝ=7;const int Ϝ=8;const int ϛ=240;const
int Ϛ=120;const double ϙ=0.25;const int Ϡ=60;const double Ϙ=0.005;const int ϖ=(int)(1/Ϙ);const double ϕ=1.0/60.0;const int
...
clock=0;int ɢ=0;bool init=false;Program(){Runtime.UpdateFrequency=UpdateFrequency.Update1;ɭ=new Ǩ(Runtime,ϖ,Ϙ);gs=new
GeneralSettings();ʎ=new Γ();gs.Context=this;gs.WCAPI=new WcPbApi();if(!gs.WCAPI.Activate(Me))gs.WCAPI=null;}void Main(string ɠ,
UpdateType ɤ){if(!init){if(!InitLoop()){return;}ʟ=-100000;assignmentState=0;ɦ=true;debugMode=false;ɭ.ǋ();clock=-1;init=true;return
;}ɭ.Ǒ();if(gs.WCAPI==null){gs.WCAPI=new WcPbApi();if(!gs.WCAPI.Activate(Me))gs.WCAPI=null;}if(debugMode)ɭ.ǥ(
"InterGridComms");if(ɠ.Length>0){if(ɠ.Equals(ϑ)){ѱ();}else{if((ɤ&ϓ)!=0){Ѽ(ɠ);}}}if(debugMode)ɭ.ǣ("InterGridComms");ɧ+=Runtime.
TimeSinceLastRun.Ticks;if((ɤ&UpdateType.Update1)==0||ɧ==0){return;}ɧ=0;clock++;if(!ɦ){if(clock%Ϡ==0){ō();}return;}if(debugMode)ɭ.ǥ(
"AutoMissileLaunch");if(clock>=ʖ){Ѵ();}if(debugMode)ɭ.ǣ("AutoMissileLaunch");if(debugMode)ɭ.ǥ("MissileReload");if(gs.Χ>0&&clock%gs.Χ==0){Ѳ(
);}if(debugMode)ɭ.ǣ("MissileReload");if(debugMode)ɭ.ǥ("ManualAimReload");if(gs.η>0&&clock%gs.η==0){ReloadMainBlocks();}if
(debugMode)ɭ.ǣ("ManualAimReload");if(debugMode)ɭ.ǥ("ManualRaycast");ʭ();if(debugMode)ɭ.ǣ("ManualRaycast");if(debugMode)ɭ.
ǥ("Designator");UpdateDesignatorTargets();if(debugMode)ɭ.ǣ("Designator");if(debugMode)ɭ.ǥ("Allies");ҕ();if(debugMode)ɭ.ǣ(
"Allies");if(targetManager.ћ()>0){ʞ=true;if(debugMode)ɭ.ǥ("RaycastTracking");if(clock>=ʚ){ʶ();}if(debugMode)ɭ.ǣ(
"RaycastTracking");if(debugMode)ɭ.ǥ("AssignTargets");if(clock%gs.ψ==0){қ();}if(debugMode)ɭ.ǣ("AssignTargets");if(debugMode)ɭ.ǥ(
"PDCAimFireReload");ѳ();if(debugMode)ɭ.ǣ("PDCAimFireReload");}else if(ʞ){ʞ=false;foreach(U ɯ in ʌ){ɯ.ø=null;ɯ.ReleaseWeapons();ɯ.ȫ();}
assignmentState=0;}if(debugMode)ɭ.ǥ("TransmitIGCMessages");if(targetManager.ћ()>0||ʁ.Count()>0){ӌ();}Ӈ();if(debugMode)ɭ.ǣ(
"TransmitIGCMessages");if(clock%gs.ͳ==0){Ү();}if(clock%Ϡ==0){ō();}ɭ.ǡ();}void ɷ(){try{ő ɿ;IMyTerminalBlock ɽ=(ʑ.Î.Count>0?ʑ.Î[0]:(
IMyTerminalBlock)Me);MyDetectedEntityInfo ɼ=new MyDetectedEntityInfo(9801,ϫ,MyDetectedEntityType.LargeGrid,ɽ.WorldMatrix.Translation+(ɽ.
WorldMatrix.Forward*0.1),ɽ.WorldMatrix,Vector3D.Zero,MyRelationsBetweenPlayerAndBlock.Enemies,ɽ.WorldAABB.Inflate(100),1);
targetManager.UpdateTarget(ref ɼ,1,out ɿ);ѱ();Ѽ(ϫ);Ѵ();ӄ(ɿ,false,true);һ(ʣ,ɿ,э.ь,null,false,true);double ɻ=gs.δ;gs.δ=0.1f;ʛ=new List<
ŀ>();ʛ.Add(new ŀ("AIM0",Me,null,null));ʭ();gs.δ=ɻ;ʛ=null;ʶ();қ();ѳ();ʶ();қ();ѳ();targetManager.і();targetManager.ј();ʞ=
false;foreach(U ɯ in ʌ){ɯ.ø=null;ɯ.ReleaseWeapons();ɯ.ȫ();}assignmentState=0;ʁ.Clear();ʀ.Clear();ӌ();ӌ();ʁ.Clear();ʀ.Clear();
ɬ=false;ɟ=0;}catch(Exception){}}bool InitLoop(){if(ɢ==0){ɮ();List<IMyRemoteControl>ɹ=new List<IMyRemoteControl>(0);
GridTerminalSystem.GetBlocksOfType(ɹ,(Ƌ)=>{if(ʍ==null){ʍ=Ƌ;}return false;});if(ʍ!=null){float ɾ=ʍ.SpeedLimit;ʍ.SpeedLimit=float.MaxValue;ɫ
=ʍ.SpeedLimit;ʍ.SpeedLimit=ɾ;}if(gs.ͼ){List<IMyTerminalBlock>ɸ=new List<IMyTerminalBlock>();GridTerminalSystem.
GetBlocksOfType(ɸ,(Ƌ)=>{return(Ƌ is IMyMechanicalConnectionBlock||Ƌ is IMyShipConnector);});List<ƶ>ɶ=new List<ƶ>();foreach(
IMyTerminalBlock ɵ in ɸ){if(ɵ is IMyMechanicalConnectionBlock){IMyMechanicalConnectionBlock ɴ=ɵ as IMyMechanicalConnectionBlock;if(ɴ.Top
!=null){ɶ.Add(new ƶ(ɴ.Top.CubeGrid,ɴ.Top.Position));}}else{IMyShipConnector ɳ=ɵ as IMyShipConnector;if(ɳ!=null&&ɳ.
OtherConnector!=null){ɶ.Add(new ƶ(ɳ.OtherConnector.CubeGrid,ɳ.OtherConnector.Position));}}}if(gs.Β){ƴ ɲ=new ƴ();ʔ=ɲ.Ǉ(new ƶ(Me.
CubeGrid,Me.Position),ɶ,gs.Λ,gs.Τ);ʕ=ɲ;ɢ=1;}else{Ɵ ɱ=new Ɵ(new ƶ(Me.CubeGrid,Me.Position),ɶ,gs.Λ,Me);ʕ=ɱ;ɢ=2;}}else{ʕ=null;}ʑ=
new Ş(new List<IMyCameraBlock>(0));targetManager=new а();ʢ=new ђ();ʡ=new SortedDictionary<double,ő>();ReloadMainBlocks();Ѳ(
);ʁ=new Queue<f>();ʀ=new Queue<ő>();gs.δ=Math.Min(Math.Max(gs.δ,1000),100000);ʓ=IGC.RegisterBroadcastListener(ϑ);ʒ=IGC.
RegisterBroadcastListener(ϒ);ɪ=Me.CubeGrid.WorldAABB.HalfExtents.Length();ɷ();}if(ɢ==1){if(ʔ.MoveNext()){Echo(
...
"RangeSweeperInterval").ToInt32(gs.ά);gs.Ϊ=ʳ.Get(І,"TargetFoundHoldTicks").ToInt32(gs.Ϊ);gs.DisplayStatusInName=ʳ.Get(І,"DisplayStatusInName").ToBoolean(false);gs.Ω=ʳ.Get(І,"UsePDCSpray").ToBoolean(gs.Ω);gs.ΰ=ʳ.
...
>ˋ;GetBlocksFromGroups(out ˋ,Ѓ,(Ƌ)=>{Ƌ.EnableRaycast=false;Ƌ.Enabled=true;return true;});if(ˋ==null){ˋ=new List<
...
WCThreatsScratchpad);foreach(var target in WCThreatsScratchpad.Keys){MyDetectedEntityInfo entityInfo=target;if(entityInfo.Type!=
MyDetectedEntityType.Unknown&&entityInfo.Relationship==MyRelationsBetweenPlayerAndBlock.Enemies&&targetManager.UpdateTarget(ref entityInfo,
clock)){ʟ=clock+gs.ϊ;}}}else{designatorTargetRR.Begin();int maxCount=(gs.MaxDesignatorUpdatesPerTick==0?designators.Count:gs.
MaxDesignatorUpdatesPerTick);for(int Q=0;Q<maxCount;Q++){Designator designator=designatorTargetRR.GetNext();if(designator!=null){
MyDetectedEntityInfo entityInfo=designator.Turret.GetTargetedEntity();if(targetManager.UpdateTarget(ref entityInfo,clock)){ʟ=clock+gs.ϊ;}ʂ=
clock;if(gs.ί){if(clock>=designator.NextResetClock){designator.Turret.ResetTargetingToDefault();designator.Turret.
EnableIdleRotation=false;designator.NextResetClock=clock+gs.ή;}}}else{break;}}if(gs.έ){if(clock<=ʂ+gs.Ϊ){designatorOperationRR.Begin();
maxCount=(gs.Ϋ==0?designators.Count:gs.Ϋ);for(int Q=0;Q<maxCount;Q++){Designator ʷ=designatorOperationRR.GetNext();if(ʷ!=null){
if(clock>=ʷ.ĳ){ʷ.į();ʷ.ĳ=clock+gs.ά;}}else{break;}}}}}}void ʶ(){ő ž=targetManager.ѧ();if(ž!=null){if(clock-ž.ļ>=gs.ύ){if(ʑ
.Î.Count>0){double ʵ=(ž.ğ==0?gs.σ:ž.ğ);if((ž.Ő-Me.WorldMatrix.Translation).LengthSquared()<=ʵ*ʵ){Vector3D Ę=ž.Ő+(ž.ę*(
clock-ž.ŏ)*ϕ);MyDetectedEntityInfo Ŵ;if(ʑ.µ(ref Ę,out Ŵ,gs.υ)){Ҍ(ref Ŵ);}ʚ=clock+gs.ϋ;}}targetManager.ѭ(ž.Ō,clock);Ҁ(ž);}}if(
gs.Ω&&gs.κ>0&&clock%gs.κ==0){
                ő ʴ = targetManager.Ѧ(); if (ʴ != null && ʴ.Ģ >= gs.ΰ * gs.ΰ)
                {
                    double ʵ = (ʴ.ğ == 0 ? gs.σ : ʴ.ğ);
                    Vector3D Ґ = ʴ.Ŏ + (ʴ.ę * (clock - ʴ.ŏ) * ϕ); if ((Ґ - Me.WorldMatrix.Translation).LengthSquared() <= ʵ * ʵ)
                    {
                        double Ҏ = Math.Sqrt(ʴ.Ģ) * 0.5 * ϙ;
                        Vector3D ҍ = new Vector3D(((ı.NextDouble() * 2) - 1) * Ҏ, ((ı.NextDouble() * 2) - 1) * Ҏ, ((ı.NextDouble() * 2) - 1) * Ҏ); Vector3D Ę = Ґ + Vector3D.
                                               TransformNormal(ҍ, ʴ.ĥ.Value);

                        if (gs.Ω && ž.Ģ >= gs.ΰ * gs.ΰ)
                        {
                            if (ž.Ě == null)
                            {
                ...
                <ī>(ϝ);
                            ...
                            Vector3D ҏ = Ę - ʴ.Ő; if (ž.Ě.Count
                          >= ϝ)
                            {
                                int ҋ = 0; double ҁ = double.MaxValue; for (int Q = 0; Q < ž.Ě.Count; Q++)
                                {
                                    if (clock > ž.Ě[Q].Ļ + ϛ) { ҋ = Q; ҁ = 0; break; }
                                    double w = (ž.Ě[Q].Ĳ -
                ҏ).LengthSquared(); if (w < ҁ) { ҋ = Q; ҁ = w; }
                                }
                                if (ҁ < double.MaxValue) { ž.Ě[ҋ] = new ī(ref ҏ, clock); }
                            }
                            else { ž.Ě.Add(new ī(ref ҏ, clock)); }
                        }
...
MyDetectedEntityInfo Ŵ){if(Ŵ.IsEmpty()){return;}if(ҵ(ref Ŵ)){ő ž;targetManager.UpdateTarget(ref Ŵ,clock,out ž);if(ž==null){return;}ž.Ģ=Ŵ.
BoundingBox.Extents.LengthSquared();if(ž.ġ==0){ž.ġ=ž.Ģ;}if(gs.Ω&&ž.Ģ>=gs.ΰ*gs.ΰ&&Ŵ.HitPosition.HasValue){if(ž.Ě==null){ž.Ě=new List
<ī>(ϝ);}Vector3D ҏ=Vector3D.TransformNormal(Ŵ.HitPosition.Value-Ŵ.Position,MatrixD.Transpose(Ŵ.Orientation));if(ž.Ě.Count
>=ϝ){int ҋ=0;double ҁ=double.MaxValue;for(int Q=0;Q<ž.Ě.Count;Q++){if(clock>ž.Ě[Q].Ļ+ϛ){ҋ=Q;ҁ=0;break;}double w=(ž.Ě[Q].Ĳ-
ҏ).LengthSquared();if(w<ҁ){ҋ=Q;ҁ=w;}}if(ҁ<double.MaxValue){ž.Ě[ҋ]=new ī(ref ҏ,clock);}}else{ž.Ě.Add(new ī(ref ҏ,clock));}
}}else{targetManager.ї(Ŵ.EntityId);targetManager.љ(Ŵ.EntityId);}}void Ҁ(ő ž){Vector3D Ȗ=Me.GetPosition();Vector3D ѿ=(ʍ!=
null?ʍ.GetShipVelocities().LinearVelocity:Vector3D.Zero);if(ž.Ģ>0&&ž.Ģ<gs.ό*gs.ό){targetManager.ї(ž.Ō);targetManager.љ(ž.Ō);
}else if(clock-ž.ŏ<=gs.φ){double Ҋ=Ҕ(ɪ,Ȗ,ѿ,ž);Ҋ+=ı.NextDouble()*0.000000000001;ž.ĩ=Ҋ;ʠ=Math.Max(Ҋ,ʠ);if(ʍ==null||ʍ.
GetShipVelocities().LinearVelocity.LengthSquared()<=gs.Ύ*gs.Ύ){if(ž.Ģ>=gs.ρ*gs.ρ){if(ž.ě==0||clock>=ž.ě+gs.Ψ){ž.ě=clock;double ґ=(ž.Ğ==0?
gs.ο:ž.Ğ);if((ž.Ő-Me.WorldMatrix.Translation).LengthSquared()<=ґ*ґ){if(ž.Ģ==0){ž.Ĝ=1;}else{ž.Ĝ=(int)Math.Ceiling(Math.Sqrt
(ž.Ģ)/Math.Max(gs.π,1));}}}}}}else{targetManager.љ(ž.Ō);}}void ҕ(){ǰ Ѡ=ʢ.ŉ();if(Ѡ!=null){if(clock-Ѡ.Ħ>gs.Ͳ){ʢ.ň(Ѡ.Ō);}}}
void қ(){switch(assignmentState){case 0:if(clock>=ʟ){if(targetManager.ћ()>0){ʟ=clock+gs.ω;ʡ.Clear();ʠ=0;List<ő>Қ=
targetManager.Ѩ();foreach(ő ž in Қ){if(!ʡ.ContainsKey(ž.ĩ)){ʡ.Add(ž.ĩ,ž);ʠ=Math.Max(ž.ĩ,ʠ);}}ʋ.ǩ();ʋ.Begin();assignmentState=1;}}
break;case 1:U ʲ=ʋ.GetNext();if(ʲ!=null){if(ʲ!=null&&!ʲ.IsDamaged){double ҙ=0;ő Ҙ=null;double җ=0;ő Ҝ=null;foreach(
KeyValuePair<double,ő>Җ in ʡ){if(ʲ.IsTargetable(Җ.Value,clock)){ҙ=Җ.Key;Ҙ=Җ.Value;if(ʲ.ÿ>0){if(Ҙ.Ģ>=ʲ.ÿ*ʲ.ÿ){җ=ҙ;Ҝ=Ҙ;break;}}else{
break;}}}if(Ҝ!=null){ҙ=җ;Ҙ=Ҝ;}if(Ҙ!=null){ʲ.ø=Ҙ;if(ʡ.ContainsKey(ҙ)){ʡ.Remove(ҙ);ʠ+=1000;ҙ=ʠ;ʡ.Add(ҙ,Ҙ);}}else{if(ʲ.ø!=null&&
!targetManager.Ѭ(ʲ.ø.Ō)){ʲ.ø=null;}ʲ.ReleaseWeapons();ʲ.ȫ();}}}else{assignmentState=0;}break;}}double Ҕ(double ɪ,Vector3D
Ȗ,Vector3D ѿ,ő ž){Vector3D ғ=ž.Ő-Ȗ;double Ҋ=ғ.Length();PlaneD Ғ;if(ѿ.LengthSquared()<0.01){Ғ=new PlaneD(Ȗ,Vector3D.
Normalize(ғ));}else{Ғ=new PlaneD(Ȗ,Ȗ+ѿ,Ȗ+ғ.Cross(ѿ));}Vector3D Ѿ=Ғ.Intersection(ref ž.Ő,ref ž.ę);Vector3D ѽ=Ѿ-ž.Ő;if(ѽ.Dot(ref ž.
ę)<0){Ҋ+=gs.μ*4;}else{double Ɣ=Math.Sqrt(ѽ.LengthSquared()/Math.Max(ž.ę.LengthSquared(),0.000000000000001));if((Ѿ-(Ȗ+(ѿ*Ɣ
))).LengthSquared()>ɪ*ɪ){Ҋ+=gs.μ*2;}else if(ž.Ģ<=0){Ҋ+=gs.μ;}else if(ž.Ģ<gs.τ*gs.τ){Ҋ+=gs.μ*3;}}if(ž.ġ>ž.Ģ){Ҋ+=gs.μ*Math.
Max(ž.ĝ,1);}else{Ҋ+=gs.μ*Math.Min(ž.ĝ,1);}return Ҋ;}void ѳ(){if(ʇ>0){if(clock>=ʆ){ʆ=clock+ʇ;}else{return;}}ʐ.Begin();int ʸ=
(ʈ==0?ʌ.Count:ʈ);for(int Q=0;Q<ʸ;Q++){U ʲ=ʐ.GetNext();if(ʲ!=null){if(clock>=ʲ.č+gs.ͺ){ʲ.č=clock;if(ʲ.Ȓ()){if(clock>=ʲ.ė+
gs.ͷ){ʲ.ė=clock;ʲ.Ȕ(clock);}}}if(ʲ.ĕ==U.Ƽ.ƻ){if(ʲ.ø!=null){ʲ.Ȑ(ʲ.ø,clock);}}else{ʲ.Ȕ(clock);}}else{break;}}}void Ѳ(){List<
IMyBlockGroup>ʮ=new List<IMyBlockGroup>();ʜ=new List<IMyProgrammableBlock>();GridTerminalSystem.GetBlockGroups(ʮ,(Ƌ)=>{return п(Ƌ,Ћ);
});foreach(IMyBlockGroup Ū in ʮ){Ū.GetBlocksOfType(ʜ,(Ƌ)=>{if(Ƌ.Enabled&&Me.IsSameConstructAs(Ƌ)){return ч.ф(Ƌ);}else{
return false;}});break;}ʮ.Clear();ʣ=new List<IMyProgrammableBlock>();GridTerminalSystem.GetBlockGroups(ʮ,(Ƌ)=>{return п(Ƌ,О);}
);foreach(IMyBlockGroup Ū in ʮ){Ū.GetBlocksOfType(ʣ,(Ƌ)=>{if(Ƌ.Enabled&&Me.IsSameConstructAs(Ƌ)){return ч.ф(Ƌ);}else{
return false;}});break;}}void ѱ(){while(ʓ.HasPendingMessage){object Ѱ=ʓ.AcceptMessage().Data;if(Ѱ is MyTuple<long,long,
Vector3D,Vector3D,double>){MyTuple<long,long,Vector3D,Vector3D,double>ѯ=(MyTuple<long,long,Vector3D,Vector3D,double>)Ѱ;if(!
targetManager.Ѭ(ѯ.Item2)||clock-targetManager.ѩ(ѯ.Item2).Ħ>=gs.χ){Ǭ ѥ=new Ǭ();ѥ.Ō=ѯ.Item2;ѥ.Ő=ѯ.Item3;ѥ.ę=ѯ.Item4;targetManager.
UpdateTarget(ѥ,clock-1,false);if(ѯ.Item5>0){ő ž=targetManager.ѩ(ѥ.Ō);if(ž!=null&&ž.Ģ==0){ž.Ģ=ѯ.Item5;}}}}}while(ʒ.HasPendingMessage)
{object Ѱ=ʒ.AcceptMessage().Data;if(Ѱ is MyTuple<long,long,Vector3D,Vector3D,double>){MyTuple<long,Vector3D,Vector3D,
double,int,long>Ѹ=(MyTuple<long,Vector3D,Vector3D,double,int,long>)Ѱ;if(!targetManager.Ѭ(Ѹ.Item1)||clock-targetManager.ѩ(Ѹ.
Item1).Ħ>=gs.χ){if((Ѹ.Item5&(int)щ.ш)==0){Ǭ ѥ=new Ǭ();ѥ.Ō=Ѹ.Item1;ѥ.Ő=Ѹ.Item2;ѥ.ę=Ѹ.Item3;targetManager.UpdateTarget(ѥ,clock-
1,false);if(Ѹ.Item4>0){ő ž=targetManager.ѩ(ѥ.Ō);if(ž!=null){if(ž.Ģ==0){ž.Ģ=Ѹ.Item4;}ž.ģ=(Ѹ.Item5&(int)щ.ģ)>0;}}}else{ǰ Ѡ=
new ǰ(Ѹ.Item1);Ѡ.Ő=Ѹ.Item2;Ѡ.ę=Ѹ.Item3;Ѡ.Ǫ=Ѹ.Item4;Ѡ.ģ=(Ѹ.Item5&(int)щ.ģ)>0;ʢ.ў(Ѡ,clock);}}}}}void Ѽ(string ѻ){string[]р=ѻ.
Split(Ϥ,StringSplitOptions.RemoveEmptyEntries);if(р.Length==0)return;string Ѻ=р[0].Trim().ToUpper();ŀ ʨ=null;if(Ѻ.StartsWith(
ϸ,StringComparison.OrdinalIgnoreCase)){int ŗ;if(Ѻ.Length==ϸ.Length){ŗ=1;}else if(!int.TryParse(Ѻ.Substring(ϸ.Length).Trim
(),out ŗ)){ŗ=0;}if(ŗ>=1&&ʙ.ContainsKey(ϸ+ŗ)){ʨ=ʙ[ϸ+ŗ];}if(ʨ!=null&&р.Length<=1){return;}}if(ʨ!=null){Ѻ=р[1].Trim().
ToUpper();}else if(ʛ?.Count>0){ʨ=ʛ[0];}switch(Ѻ){case М:bool ѹ=н(р,К);List<IMyProgrammableBlock>ѷ=(ѹ?ʣ:ʜ);if(н(р,Л)){ő ʴ=
targetManager.Ѧ();if(ʴ!=null){һ(ѷ,ʴ,ʨ?.ř??э.ь,ʨ?.ī,ѹ);}}else if(ʨ!=null){bool ѵ=н(р,Й);if(ʨ.Ś>0&&targetManager.Ѭ(ʨ.Ś)){ő Ѷ=
targetManager.ѩ(ʨ.Ś);if(Ѷ!=null){if(ѵ){Ѷ.Ğ=Math.Max(ʨ.Ř,gs.ο);}һ(ѷ,Ѷ,ʨ.ř,ʨ.ī,ѹ);}}else{ʨ.Ŝ=true;ʨ.Ś=-1;ʨ.ī=Vector3D.Zero;ʨ.Ő=ʨ.ľ.
WorldMatrix.Translation+(ʨ.œ()*ʨ.Ř);ʨ.ğ=Math.Max(ʨ.Ř,gs.σ);ʨ.Ğ=(ѵ?Math.Max(ʨ.Ř,gs.ο):0);һ(ѷ,ʨ,ʨ.ř,ʨ.ī,ѹ);}}break;case И:if(ʨ!=null)
{if(ʨ.Ŝ&&ʨ.Ś==-1){ʨ.Ŝ=false;ʨ.ī=Vector3D.Zero;}else{bool ѵ=н(р,Й);ʨ.Ŝ=true;ʨ.Ś=-1;ʨ.ī=Vector3D.Zero;ʨ.Ő=ʨ.ľ.WorldMatrix.
Translation+(ʨ.œ()*ʨ.Ř);ʨ.ğ=Math.Max(ʨ.Ř,gs.σ);ʨ.Ğ=(ѵ?Math.Max(ʨ.Ř,gs.ο):0);}}break;case З:if(ʨ!=null){ʨ.Ŝ=false;ʨ.Ś=-1;ʨ.ī=
Vector3D.Zero;}break;case Ж:if(ʨ!=null&&р.Length>=3){switch(р[2].ToUpper().Trim()){case Е:ʨ.ř=э.ь;break;case Д:ʨ.ř=э.ы;break;
case Г:ʨ.ř=э.ъ;break;case В:if(р.Length>=4){double ǲ;if(double.TryParse(р[3].Trim(),out ǲ)){ʨ.Ř=Math.Min(Math.Max(ǲ,1000),
100000);;}}break;}}break;case Џ:if(ʨ!=null){ʨ.ř=(э)(((int)ʨ.ř+1)%3);}break;case Б:if(ʨ!=null){ʨ.Ř=Math.Min(Math.Max(ʨ.Ř+1000,
1000),100000);;}break;case А:if(ʨ!=null){ʨ.Ř=Math.Min(Math.Max(ʨ.Ř-1000,1000),100000);;}break;case Ў:ɦ=!ɦ;if(gs.DisplayStatusInName){var s=Me.CustomName.Split('|');if(s.Length==2){Me.CustomName = s[0] + "|" + (ɦ?$" DDS ON {ʌ.Count(Ƌ=>{return!Ƌ.IsDamaged;})}":" DDS OFF");}};if(!ɦ){foreach(U ʲ
in ʌ){ʲ.ø=null;ʲ.ReleaseWeapons(true);ʲ.ȫ();}}break;case Ѝ:ɦ=true;break;case Ќ:ɦ=false;foreach(U ʲ in ʌ){ʲ.ø=null;ʲ.
ReleaseWeapons(true);ʲ.ȫ();}break;case Ϸ:gs.ς=!gs.ς;break;case ϡ:gs.ς=true;break;case Ϗ:gs.ς=false;break;case ϟ:debugMode=!debugMode;
break;}}void Ѵ(){if(gs.ς){if(targetManager.ћ()>0){ő ž=targetManager.ѧ();if(ž!=null){if(ž.Ĝ>0&&targetManager.Ѭ(ž.Ō)){ž.Ĝ--;
bool Ӄ=(ž.Ĝ<=0?true:(ı.NextDouble()<=gs.ν));ӄ(ž,Ӄ);ʖ=clock+gs.λ;}}}}}void ӄ(ő ž,bool Ӄ=false,bool ӂ=false){
IMyProgrammableBlock Ӂ=null;double ǭ=double.MaxValue;foreach(IMyProgrammableBlock ɵ in ʜ){if(ɵ.IsWorking&&GridTerminalSystem.GetBlockWithId(
ɵ.EntityId)!=null){double Ӆ=(ɵ.WorldMatrix.Translation-ž.Ő).LengthSquared();if(Ӆ<ǭ){ǭ=Ӆ;Ӂ=ɵ;}}}if(Ӂ!=null&&!ӂ){f ʺ=new f(
);ʺ.e=Һ();ʺ.m=ž;ʺ.d=clock+gs.ͱ;ʺ.X=Ӄ;ʁ.Enqueue(ʺ);f ҿ=new f();ҿ.m=ž;ҿ.d=int.MaxValue;ҿ.W=true;ʁ.Enqueue(ҿ);MyTuple<long,
long,long,int,float>Ҿ=new MyTuple<long,long,long,int,float>();Ҿ.Item1=Me.EntityId;Ҿ.Item2=ʺ.e;Ҿ.Item3=Me.EntityId;Ҿ.Item4|=(
int)я.ю;Ҿ.Item5=(Ӄ?(float)(Math.Sqrt(ž.Ģ)*0.5*gs.ξ):0f);bool ҽ=IGC.SendUnicastMessage(Ӂ.EntityId,"",Ҿ);if(!ҽ){MyIni Ҽ=new
MyIni();if(Ӂ.CustomData.Length>0)Ҽ.TryParse(Ӂ.CustomData);Ҽ.Set(Ѕ,"UniqueId",Ҿ.Item2);Ҽ.Set(Ѕ,"GroupId",Ҿ.Item3);if(Ӄ){Ҽ.Set(
Ѕ,"OffsetTargeting",Ҿ.Item4);Ҽ.Set(Ѕ,"RandomOffsetAmount",Ҿ.Item5);}Ӂ.CustomData=Ҽ.ToString();Ӂ.TryRun("FIRE:"+Me.
EntityId);}ʜ.Remove(Ӂ);}}void һ(List<IMyProgrammableBlock>ѷ,ő ž,э Ӏ=э.ь,Vector3D?ҏ=null,bool ӆ=false,bool ӂ=false){for(int Q=0;Q
<ѷ.Count;Q++){IMyProgrammableBlock Ӎ=ѷ[Q];if(Ӎ.IsWorking&&GridTerminalSystem.GetBlockWithId(Ӎ.EntityId)!=null&&!ӂ){f ʺ=
new f();ʺ.e=Һ();ʺ.m=ž;ʺ.d=clock+gs.ε;if(ӆ&&п(Ӎ,Н))ʺ.V=Ӎ;ʁ.Enqueue(ʺ);MyIni Ҽ=null;MyTuple<long,long,long,int,float>Ҿ=new
MyTuple<long,long,long,int,float>();Ҿ.Item1=Me.EntityId;Ҿ.Item2=ʺ.e;Ҿ.Item3=Me.EntityId;switch(Ӏ){case э.ы:Ҿ.Item4|=(int)я.ю;Ҽ=
new MyIni();if(Ӎ.CustomData.Length>0)Ҽ.TryParse(Ӎ.CustomData);Ҽ.Set(Ѕ,"ProbeOffsetVector",Ҹ(ҏ!=null?ҏ.Value:Vector3D.Zero))
;Ӎ.CustomData=Ҽ.ToString();break;case э.ъ:Ҿ.Item4|=(int)я.ю;Ҿ.Item5=(float)(Math.Sqrt(ž.Ģ)*0.5*gs.ξ);break;default:Ҿ.
Item5=0f;break;}bool ҽ=IGC.SendUnicastMessage(Ӎ.EntityId,"",Ҿ);if(!ҽ){if(Ҽ==null){Ҽ=new MyIni();if(Ӎ.CustomData.Length>0)Ҽ.
TryParse(Ӎ.CustomData);}Ҽ.Set(Ѕ,"UniqueId",Ҿ.Item2);Ҽ.Set(Ѕ,"GroupId",Ҿ.Item3);switch(Ӏ){case э.ы:Ҽ.Set(Ѕ,"OffsetTargeting",Ҿ.
Item4);Ҽ.Set(Ѕ,"ProbeOffsetVector",Ҹ(ҏ!=null?ҏ.Value:Vector3D.Zero));break;case э.ъ:Ҽ.Set(Ѕ,"OffsetTargeting",Ҿ.Item4);Ҽ.Set(
Ѕ,"RandomOffsetAmount",Ҿ.Item5);break;}Ӎ.CustomData=Ҽ.ToString();Ӎ.TryRun("FIRE:"+Me.EntityId);}if(ѷ.Count==1){ѷ.Clear();
}else{ѷ[Q]=ѷ[ѷ.Count-1];ѷ.RemoveAt(ѷ.Count-1);}break;}}}void ӌ(){ɬ=false;if(ɟ==0){Ӌ();if(!ɬ){ӈ();}ɟ=1;}else{ӈ();if(!ɬ){Ӌ(
);}ɟ=0;}}void Ӌ(){while(ʁ.Count>0){f ƿ=ʁ.Dequeue();if(clock<=ƿ.d){if(!(ƿ.m.Ō==-1||targetManager.Ѭ(ƿ.m.Ō))){break;}if(
clock>=ƿ.Y){if(ƿ.V==null){MyTuple<bool,long,long,long,Vector3D,Vector3D>ӊ=new MyTuple<bool,long,long,long,Vector3D,Vector3D>(
);ӊ.Item1=ƿ.W;ӊ.Item2=(ƿ.W?Me.EntityId:ƿ.e);ӊ.Item3=0;ӊ.Item4=ƿ.m.Ō;ӊ.Item5=ƿ.m.Ő+(ƿ.m.ę*(clock-ƿ.m.ŏ+1)*ϕ);ӊ.Item6=ƿ.m.ę
;IGC.SendBroadcastMessage(ƿ.W?ϗ:ϐ,ӊ);}else{Vector3D Ę=ƿ.m.Ő+(ƿ.m.ę*(clock-ƿ.m.ŏ+1)*ϕ);MyIni Ӊ=new MyIni();Ӊ.Set(Ϣ,
"EntityId",ƿ.m.Ō);Ӊ.Set(Ϣ,"PositionX",Ę.X);Ӊ.Set(Ϣ,"PositionY",Ę.Y);Ӊ.Set(Ϣ,"PositionZ",Ę.Z);Ӊ.Set(Ϣ,"VelocityX",ƿ.m.ę.X);Ӊ.Set(Ϣ,
"VelocityY",ƿ.m.ę.Y);Ӊ.Set(Ϣ,"VelocityZ",ƿ.m.ę.Z);ƿ.V.TryRun(Ϣ+Ӊ.ToString());}ƿ.Y=clock+gs.Ώ;ɬ=true;}if(!ƿ.W){ʁ.Enqueue(ƿ);}break;}
}}void ӈ(){if(clock>=ɩ&&ʀ.Count==0){if(targetManager.ћ()>0){List<ő>Қ=targetManager.Ѩ();Қ.Sort(Ͱ);foreach(ő ž in Қ){ʀ.
Enqueue(ž);}}ɩ=clock+gs.ζ;}while(ʀ.Count>0){ő ž=ʀ.Dequeue();if(targetManager.ѫ(ž.Ō,clock-gs.φ)){MyTuple<long,Vector3D,Vector3D,
double,int,long>Ѹ=new MyTuple<long,Vector3D,Vector3D,double,int,long>();Ѹ.Item1=ž.Ō;Ѹ.Item2=ž.Ő+(ž.ę*(clock-ž.ŏ+1)*ϕ);Ѹ.Item3=
ž.ę;Ѹ.Item4=ž.Ģ;if(ž.ģ)Ѹ.Item5|=(int)щ.ģ;Ѹ.Item6=0;IGC.SendBroadcastMessage(ϒ,Ѹ);ɬ=true;break;}}}void Ӈ(){if(clock>=ɨ){
...
);ɨ=clock+gs.ζ;}}void Ү(){foreach(ŀ ʨ in ʛ){if(ʨ.ō!=null){IMyTextSurface Ҭ=ʨ.ō;Vector2 ҫ;Vector2 Ҫ;if(Ҭ.SurfaceSize.X==Ҭ.
SurfaceSize.Y){ҫ=(Ҭ.TextureSize-Ҭ.SurfaceSize)*0.5f;Ҫ=new Vector2(Ҭ.SurfaceSize.X)*0.0009765625f;}else if(Ҭ.SurfaceSize.X>Ҭ.
SurfaceSize.Y){ҫ=(Ҭ.TextureSize-Ҭ.SurfaceSize+new Vector2(Ҭ.SurfaceSize.X-Ҭ.SurfaceSize.Y,0f))*0.5f;Ҫ=new Vector2(Ҭ.SurfaceSize.Y)*
0.0009765625f;}else{ҫ=(Ҭ.TextureSize-Ҭ.SurfaceSize+new Vector2(0f,Ҭ.SurfaceSize.Y-Ҭ.SurfaceSize.X))*0.5f;Ҫ=new Vector2(Ҭ.SurfaceSize.
Y)*0.0009765625f;}List<MySprite>ҩ=new List<MySprite>();ҩ.Add(new MySprite(SpriteType.TEXTURE,"SquareSimple",Ҭ.TextureSize
...
TextAlignment.CENTER,ҟ));Ҟ+=ҭ;if(ʨ.Ś>0){ҝ="Target Locked";Ҥ=Ϫ;ү=ϩ;}else if(ʨ.Ŝ){ҝ="Seeking";Ҥ=ϴ;ү=Ϭ;}else{ҝ="No Target";Ҥ=Ϯ;ү=ϭ;}ҩ.
...
if(ʨ.Ś>0){ҝ=ҹ(ʨ.Ś);Ҥ=Ϩ;ү=ϧ;}else{ҝ="-";Ҥ=Ϯ;ү=ϭ;}ҩ.Add(new MySprite(SpriteType.TEXTURE,"SquareSimple",Ҟ+Ҧ,Ҩ,ү));ҩ.Add(new
...
.ContentType=ContentType.SCRIPT;Ҭ.Script="";MySpriteDrawFrame Ҵ=Ҭ.DrawFrame();Ҵ.AddRange(ҩ);Ҵ.Dispose();}}}void ō(){var s=Me.CustomName.Split('|');if(s.Length==2){Me.CustomName = s[0] + "|" + (ɦ?$" DDS ON {ʌ.Count(Ƌ=>{return!Ƌ.IsDamaged;})}":" DDS OFF");}bool
wc=gs.WCAPI!=null;ǟ.Clear();ɣ=(ɣ+1)%8;ǟ.AppendLine($"====[ Diamond Dome System ]==={ϣ[ɣ]}");ǟ.AppendLine(
$"<<Version {Ј}>>\n");ǟ.AppendLine($"WeaponCoreAPI : {wc}\n");ǟ.AppendLine($"PDCs : {ʌ.Count(Ƌ=>{return!Ƌ.IsDamaged;})}");ǟ.AppendLine(
$"Designators : {designators.Count}");ǟ.AppendLine($"Raycast Cameras : {ʉ.Count}");ǟ.AppendLine($"Guided Missiles : {ʜ.Count}");ǟ.AppendLine(
$"Guided Torpedos : {ʣ.Count}");if(ɦ){ǟ.AppendLine($"Tracked Targets : {targetManager.ћ()}");}ǟ.
...
Vector3D.Zero;}Ŋ.є=Ƿ; ž.Ģ=Ŵ.BoundingBox.Extents.LengthSquared()*0.5;return false;}else{ž=new ő(Ŵ.EntityId);ž.Ő=Ŵ.Position;ž.ę=ž.ħ=Ŵ.Velocity;ž.ŏ=ž.Ħ=Ƿ;ņ Ŋ=new ņ();Ŋ.Ō=ž.Ō;Ŋ.m=
ž;Ŋ.є=Ƿ;Я.Add(Ŵ.EntityId,Ŋ);Ю.Add(Ŋ); ž.Ģ=Ŵ.BoundingBox.Extents.LengthSquared()*0.5;return true;}}public bool UpdateTarget(Ǭ ѥ,int Ƿ,bool Ѥ=true){if(Э.Contains(ѥ.Ō)){
...
}class Į{public double ĭ=400;public double Ĩ=0;public double Ī=400;public double Ĭ=1550;public double ŝ=0;public double Ƈ=
...
(ȋ<1){Ċ=ϕ;}else{Ċ=ϕ/ȋ*(Đ*(ž.Ŏ-Ô.WorldMatrix.Translation).Length());}ĉ=0;}ĉ+=(ĸ-ċ)*Ċ;ċ=ĸ;return ž.Ŏ+Vector3D.Lerp(Ď,Č,ĉ);}private bool CheckAndSetAllRotors(double aimAzimuth,double aimElevation,int
